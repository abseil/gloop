...
#define SHLD(n) byte = ((byte << (n * 7)) | (ones >> (64 - (n * 7))))
  ...
  // NOLINTBEGIN(google3-runtime-inline-assembly): b/0
#define SHLD_SIGN(n)                  \
  asm("shldq %3, %2, %1"              \
      : "=@ccs"(sign_bit), "+r"(byte) \
      : "r"(ones), "i"(n * 7))
  // NOLINTEND(google3-runtime-inline-assembly)
#else
#define SHLD_SIGN(n)                         \
  do {                                       \
    SHLD(n);                                 \
...
const char* Varint::Parse64WithLimit(const char* p, const char* l,
  ...
  if (reinterpret_cast<uintptr_t>(p) + kMax64 <=
    ...
    // clang-format on
    // NOLINTEND(readability/braces)
    // In this slow path, our range started at 9 bytes or smaller, so if
    // we're not done, we're out of buffer.  We don't need to handle the
    // 10th byte since there is never a 10th byte.
    // Use DCHECK instead of DCHECK_EQ because ptr can be a register variable.
    DCHECK(ptr == limit);
    return nullptr;
  done:
    *OUTPUT = result;
