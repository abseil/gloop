...
#ifndef THIRD_PARTY_GLOOP_UTIL_CODING_VARINT_H_
#define THIRD_PARTY_GLOOP_UTIL_CODING_VARINT_H_
...
class Varint {
 public:
  ...
  // Attempts to parse a varint-encoded value from a prefix of the bytes in
  // [ptr,limit-1]. Never reads a character at or beyond limit.  If a
  // valid/terminated varint was found in the range, stores it in *OUTPUT and
  // returns a pointer just past the last byte of the varint. Else returns
  // nullptr.  On success, "result <= limit".
  ...
  // Attempts to skip a varint-encoded value from a prefix of the bytes in
  // [ptr,limit-1]. Never reads a character at or beyond limit.  If a
  // valid/terminated varint was found in the range, returns a pointer just
  // past the last byte of the varint. Else returns nullptr.
  static const char* Skip32WithLimit(const char* ptr, const char* limit);
  static const char* Skip64WithLimit(const char* ptr, const char* limit);
  ...
  // NOTE: The code does NO error checking, it assumes all the
...
// NOLINTBEGIN(readability/braces) Many false positives.
...
inline const char* Varint::Skip32WithLimit(const char* p, const char* l) {
  const unsigned char* ptr = reinterpret_cast<const unsigned char*>(p);
...
inline const char* Varint::Skip64WithLimit(const char* p, const char* l) {
  const unsigned char* ptr = reinterpret_cast<const unsigned char*>(p);
  const unsigned char* limit = reinterpret_cast<const unsigned char*>(l);
  if (ptr >= limit) return nullptr;
  if (*ptr++ < 128) return reinterpret_cast<const char*>(ptr);
...
#endif  // THIRD_PARTY_GLOOP_UTIL_CODING_VARINT_H_
