...
class Writer {
 ...
 private:
  ...
  struct ValueInfo {
    ...
    // Index into values_remaining_at_rank_of_width_.data()[][] (see
  ...
  // original test data (some token IDs and
  // attributes).
  //
  ...
  gtl::UniqueArray<ValueFreq> sorted_values_repeated_;

  // Additional information about the values in sorted_values_repeated_
  // (see ValueInfo above). values_remaining_at_rank_of_width_.data()[
  // value_info_.data()[i].values_remaining_rank ][w] is the number of
  // repeated values of width w, excluding all values at indices < i of
  // sorted_values_repeated_. value_info_.data()[i].values_remaining_rank is
  ...
  gtl::UniqueArray<ValueInfo> value_info_;
  gtl::UniqueArray<int64_t[65]> values_remaining_at_rank_of_width_;
...
void Writer<raw_type>::InitStats() {
  ...
  if (values_repeated_ > 0) {
    sorted_values_repeated_ =
        gtl::MakeUniqueArrayForOverwrite<ValueFreq>(values_repeated_);
  }
  int64_t i = 0;
  ...
  ForEachValueFreq([&](raw_type value, int64_t freq) {
    ...
    if (freq == 1) {
      ...
      sorted_values_repeated_.data()[i++] = ValueFreq{value, freq};
    }
  });

  CHECK_EQ(i, values_repeated_);

  std::sort(sorted_values_repeated_.data(),
            sorted_values_repeated_.data() + values_repeated_,
  ...
  value_info_ =
      gtl::MakeUniqueArrayForOverwrite<ValueInfo>(values_annotated + 1);
  int64_t positions_prior = 0;
  int width_max_prior = 0, ranks = 0;
  uint64_t value_max_prior = 0, value_threshold = 0;

  value_info_.data()[0].positions_prior = positions_prior;
  value_info_.data()[0].value_max_prior = value_max_prior;
  value_info_.data()[0].width_max_prior = width_max_prior;
  value_info_.data()[0].values_remaining_rank = ranks++;

  for (int64_t i = 1; i <= values_annotated; ++i) {
    ValueFreq vf = sorted_values_repeated_.data()[i - 1];
    value_max_prior =
    ...
    value_info_.data()[i].value_max_prior = value_max_prior;
    value_info_.data()[i].width_max_prior = width_max_prior;
    positions_prior += vf.freq;
    value_info_.data()[i].positions_prior = positions_prior;
    value_info_.data()[i].values_remaining_rank =
        absl::popcount(static_cast<uint32_t>(i)) > kNumProfiles - 1 ? -1
                                                                    : ranks++;
  }

  values_remaining_at_rank_of_width_ =
      gtl::MakeUniqueArrayForOverwrite<int64_t[65]>(ranks);

  for (int64_t i = values_repeated_;;) {
    if (i <= values_annotated) {
      const int rank = value_info_.data()[i].values_remaining_rank;
      if (rank >= 0) {
        memcpy(values_remaining_at_rank_of_width_.data()[rank] + width_min_,
    ...
    if (--i < 0) break;
    ValueFreq vf = sorted_values_repeated_.data()[i];
...
bool Writer<raw_type>::FindBestLexicons(
  ...
  const ValueInfo* const vi = value_info_.data() + values_covered;
  ...
  const int64_t* const values_remaining_of_width =
      values_remaining_at_rank_of_width_.data()[vi->values_remaining_rank];
    ...
    for (int w = width_start, values_lex = kint64one << w;
      ...
      if (vi->value_max_prior == next_vi->value_max_prior) {
        ...
        const int64_t* const next_values_remaining_of_width =
            values_remaining_at_rank_of_width_
                .data()[next_vi->values_remaining_rank];
...
void Writer<raw_type>::EncodeLexicons() {
  ...
  if (best_width_lex_[profile] < 0) {
    ...
    gtl::UniqueArray<raw_type> sorted_values_lex;
    if (values_lex_last > 0) {
      sorted_values_lex =
          gtl::MakeUniqueArrayForOverwrite<raw_type>(values_lex_last);
    }
    int64_t i = 0;
    while (i + values_lex_nonlast < values_repeated_) {
      sorted_values_lex.data()[i] =
          sorted_values_repeated_.data()[values_lex_nonlast + i].value;
      ++i;
    }
    ...
    if (width_direct_max < 64) {
      ForEachValueFreq([&](uint64_t value, int64_t freq) {
        ...
        if (width_direct_max < width_min_ || value >> width_direct_max > 0) {
          sorted_values_lex.data()[i++] = value;
        }
      });
    }

    CHECK_EQ(i, values_lex_last);
    std::sort(sorted_values_lex.data(),
              sorted_values_lex.data() + values_lex_last);
    ...
    for (i = values_lex_last - 1; i >= 0; --i) {
      ...
      const raw_type value = sorted_values_lex.data()[--i_for_sublex[sublex]];
  ...
  while (--profile >= 0) {
    ...
    while (i > start) {
      const raw_type value = sorted_values_repeated_.data()[--i].value;
...
uint64_t ShrunkArray::Reader::SharedGet(size_t position) const {
  return static_cast<const impl::Reader*>(this)->SharedGet(position);
}
...
void ShrunkArray::UnalignedLittleEndianReader::Bind(const void* shrunk_array,
  ...
  static_cast<impl::UnalignedLittleEndianReader*>(this)->Bind(
...
void ShrunkArray::UnalignedLittleEndianReader::Bind1(const void* shrunk_array,
                                                     const void* decode_key) {
  static_cast<impl::UnalignedLittleEndianReader*>(this)->Bind1(
      impl::UnalignedLittleEndianReader::const_uint64_ptr(
          reinterpret_cast<const char*>(shrunk_array)),
      impl::UnalignedLittleEndianReader::const_uint64_ptr(
          reinterpret_cast<const char*>(decode_key)));
...
uint64_t ShrunkArray::UnalignedLittleEndianReader::Get(size_t position) {
  return static_cast<impl::UnalignedLittleEndianReader*>(this)->Get(position);
}

uint64_t ShrunkArray::UnalignedLittleEndianReader::SharedGet(
    size_t position) const {
  return static_cast<const impl::UnalignedLittleEndianReader*>(this)->SharedGet(
      position);
}
