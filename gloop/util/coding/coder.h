...
#ifndef THIRD_PARTY_GLOOP_UTIL_CODING_CODER_H_
#define THIRD_PARTY_GLOOP_UTIL_CODING_CODER_H_
 ...
 private:
  ...
  struct Writer {
    ...
    ~Writer() {
      ...
      DCHECK_GE(enc->buf_, enc->orig_);
      DCHECK_LE(enc->buf_, enc->limit_);
    }
...
// TODO: Remove when LLVM detects and optimizes this case.
...
inline Decoder::Decoder(const void* b, size_t maxn) { reset(b, maxn); }

inline void Decoder::reset(const void* b, size_t maxn) {
  ABSL_HARDENING_ASSERT(b != nullptr || maxn == 0);
...
inline unsigned char const* Decoder::ptr() const { return buf_; }
...
inline float Decoder::getfloat() { return absl::bit_cast<float>(get32()); }

inline double Decoder::getdouble() { return absl::bit_cast<double>(get64()); }
...
#endif  // THIRD_PARTY_GLOOP_UTIL_CODING_CODER_H_
