...
TYPED_TEST_P(BitmapTest, Distance) {
  BasicBitmap<TypeParam> map(100);
  BasicBitmap<TypeParam> map2(100);
  for (int i = 0; i < 100; i++) map.Set(i, (i % 7) == 0);
  for (int i = 0; i < 100; i++) map2.Set(i, (i % 3) == 0);
  ...
  EXPECT_EQ(map.Distance(map2), expected_dist);
  EXPECT_EQ(map2.Distance(map), expected_dist);
}
...
REGISTER_TYPED_TEST_SUITE_P(
    BitmapTest, TestRange, Distance, RequiredArraySize, OverAllocate,
