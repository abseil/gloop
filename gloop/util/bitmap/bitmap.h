...
#ifndef THIRD_PARTY_GLOOP_UTIL_BITMAP_BITMAP_H_
#define THIRD_PARTY_GLOOP_UTIL_BITMAP_BITMAP_H_
...
class BasicBitmap {
 ...
 public:
  ...
  // Cast to unsigned because popcount and countr_zero don't work with signed
  // char when backed by libc++ implementations (even when using
  // -funsigned-char).
  using Word = typename std::make_unsigned_t<
      typename remove_atomic<W>::type>;  // packed bit
                                         // storage type.
  ...
  // Returns the Hamming distance between "this" and "other". This is the number
  // of positions at which the corresponding bits are different.
  // REQUIRES: "bits() == other.bits()" (i.e. the bitmaps are the same size)
  size_type HammingDistance(const BasicBitmap& other) const;
...
template <typename W>
typename BasicBitmap<W>::size_type BasicBitmap<W>::HammingDistance(
...
#endif  // THIRD_PARTY_GLOOP_UTIL_BITMAP_BITMAP_H_
