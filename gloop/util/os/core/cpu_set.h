...
#ifndef THIRD_PARTY_GLOOP_UTIL_OS_CORE_CPU_SET_H_
#define THIRD_PARTY_GLOOP_UTIL_OS_CORE_CPU_SET_H_
...
inline std::string CpuSetToHexString(const cpu_set_t* cpu_set) {
...
// CPU_ZERO.
inline void CpuSetClear(cpu_set_t* cpu_set) { CPU_ZERO(cpu_set); }
...
inline void CpuSetClearSubset(const cpu_set_t* in, const cpu_set_t* to_clear,
                              cpu_set_t* result) {
  CpuSetClearSubset(in, to_clear, result, nullptr);
}
...
// the cpu_set_t.
cpu_set_t ProtobufToCpuSet(const google::protobuf::RepeatedField<uint64_t>& pb);
...
#endif  // THIRD_PARTY_GLOOP_UTIL_OS_CORE_CPU_SET_H_
