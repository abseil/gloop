...
#include "gloop/util/floatops/castops.h"
...
TEST(CastOpsTest, SaturatingFloatToInt) {
  ...
  EXPECT_EQ(127, (SaturatingFloatToInt<float, Schar>(FLOAT_INFI)));
  EXPECT_EQ(-128, (SaturatingFloatToInt<float, Schar>(FLOAT_NINFI)));
  EXPECT_EQ(0, (SaturatingFloatToInt<float, Schar>(FLOAT_NAN)));
  EXPECT_EQ(127, (SaturatingFloatToInt<double, Schar>(DOUBLE_INFI)));
  EXPECT_EQ(-128, (SaturatingFloatToInt<double, Schar>(DOUBLE_NINFI)));
  EXPECT_EQ(0, (SaturatingFloatToInt<double, Schar>(DOUBLE_NAN)));
  ...
  EXPECT_EQ(255, (SaturatingFloatToInt<float, Uchar>(FLOAT_INFI)));
  EXPECT_EQ(0, (SaturatingFloatToInt<float, Uchar>(FLOAT_NINFI)));
  EXPECT_EQ(0, (SaturatingFloatToInt<float, Uchar>(FLOAT_NAN)));
  EXPECT_EQ(255, (SaturatingFloatToInt<double, Uchar>(DOUBLE_INFI)));
  EXPECT_EQ(0, (SaturatingFloatToInt<double, Uchar>(DOUBLE_NINFI)));
  EXPECT_EQ(0, (SaturatingFloatToInt<double, Uchar>(DOUBLE_NAN)));
...
TEST(CastOpsTest, DoubleToFloat) {
  // NaN
  EXPECT_TRUE(
      std::isnan(DoubleToFloat(std::numeric_limits<float>::quiet_NaN())));
  ...
  EXPECT_EQ(-std::numeric_limits<float>::infinity(),
            DoubleToFloat(-std::numeric_limits<double>::infinity()));
  EXPECT_EQ(-std::numeric_limits<float>::infinity(),
            DoubleToFloat(2.0 * static_cast<double>(
...
TEST(CastOpsTest, DoubleToFiniteFloat) {
  // NaN
  EXPECT_TRUE(
      std::isnan(DoubleToFiniteFloat(std::numeric_limits<float>::quiet_NaN())));
  ...
  EXPECT_EQ(
      std::numeric_limits<float>::lowest(),
      DoubleToFiniteFloat(
          2.0 * static_cast<double>(std::numeric_limits<float>::lowest())));
}
...
TEST(CastOptsTest, ToUint64) {
  ...
  EXPECT_EQ(UINT64_MAX - 2047, ToUint64<double>(DOUBLE_UINT64_MAX - 2048));
...
#define CHECK_SMALLINT_INFI(IntType)                 \
  EXPECT_EQ(0, (To##IntType<float>(FLOAT_INFI)));    \
  EXPECT_EQ(0, (To##IntType<float>(FLOAT_NINFI)));   \
  EXPECT_EQ(0, (To##IntType<float>(FLOAT_NAN)));     \
  EXPECT_EQ(0, (To##IntType<double>(DOUBLE_INFI)));  \
  EXPECT_EQ(0, (To##IntType<double>(DOUBLE_NINFI))); \
  EXPECT_EQ(0, (To##IntType<double>(DOUBLE_NAN)));
