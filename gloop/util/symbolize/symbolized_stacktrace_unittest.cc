...
TEST(util, CurrentStackTraceWithUrlWorks) {
  ...
  const auto str2 = util::CurrentStackTrace(true);
  // Open source doesn't support URL generation as it depends on internal
  // libraries.
  (void)str2;
}
...
ABSL_ATTRIBUTE_NO_TAIL_CALL  // Prevent tail call optimization
