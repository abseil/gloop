...
TEST_P(SymbolMapTest, SelfSymbosArePresent) {
  ...
  const char* foo_symbol = symbol_map.GetSymbolAtPosition(p_foo);
  ASSERT_NE(foo_symbol, nullptr);
  EXPECT_TRUE(absl::StartsWith(foo_symbol, "_ZL3Foov"));
  const auto p_bar = reinterpret_cast<uint64_t>(&Bar);
  const char* bar_symbol = symbol_map.GetSymbolAtPosition(p_bar);
  ASSERT_NE(bar_symbol, nullptr);
  EXPECT_TRUE(absl::StartsWith(bar_symbol, "_ZL3Barv"));
}
