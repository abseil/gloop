...
#ifndef THIRD_PARTY_GLOOP_UTIL_INTOPS_LOSSLESS_CONVERT_H_
#define THIRD_PARTY_GLOOP_UTIL_INTOPS_LOSSLESS_CONVERT_H_
...
//   int32 y;
//
//   if (util_intops::LosslessConvert(x, &y)) {
//     LOG(INFO) << "Converted to: " << y;
//   }
//
template <typename InType, typename OutType>
bool LosslessConvert(InType in_val, OutType* out);

// Variant of LosslessConvert returning a failed Status on error.
//
// Example usage:
//
//   float x = 5.0;
...
LosslessConvert(InType in_val, OutType* out) {
  ...
  if (!util::castops::InRange<InType, OutType>(in_val)) {
    return false;
  }
...
LosslessConvert(InType in_val, OutType* out) {
  ...
  *out = static_cast<OutType>(in_val);
  ...
  return util::castops::InRange<OutType, InType>(*out) &&
...
[[nodiscard]] bool LosslessConvert(InType in_val, OutType* out) {
  ...
  static_assert(!std::is_integral<InType>::value || sizeof(InType) <= 8,
                "128-bit integer types are unsupported");
  static_assert(!std::is_integral<OutType>::value || sizeof(OutType) <= 8,
                "128-bit integer types are unsupported");
...
absl::StatusOr<OutType> LosslessConvert(InType in) {
  OutType out;
  if (!LosslessConvert(in, &out)) {
    return absl::OutOfRangeError(absl::StrFormat(
        "Value (%s)%v cannot be losslessly converted to %s",
        internal::GetTypeName<InType>(), in, internal::GetTypeName<OutType>()));
  }
  return out;
}
...
#endif  // THIRD_PARTY_GLOOP_UTIL_INTOPS_LOSSLESS_CONVERT_H_
