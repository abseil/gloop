...
using SupportedStrongIntTypes =
    ::testing::Types<StrongInt8, StrongUInt8, StrongInt16, StrongUInt16,
                     ...
                     StrongLong, StrongUInt128, StrongInt128>;
...
TYPED_TEST(StrongIntTest, TestStrongIntRangeEmptyRanges) {
  using T = typename TestFixture::T;
  const T begin{0};
  const T end{1};
  const T past_end{2};

  // Verify that a loop where begin < end has the right number of iterations.
  {
    int num_iters = 0;
    for ([[maybe_unused]] const auto x : MakeStrongIntRange(begin, end)) {
      num_iters++;
    }
    EXPECT_EQ(num_iters, 1);
  }

  // Verify that a loop where begin == end has zero iterations.
  for ([[maybe_unused]] const auto x : MakeStrongIntRange(end, end)) {
    FAIL() << "unexpectedly iterating over empty range";
  }

  // Verify that a loop where begin == end + 1 has zero iterations.
  for ([[maybe_unused]] const auto x : MakeStrongIntRange(past_end, end)) {
    FAIL() << "unexpectedly iterating over empty range";
  }
}
