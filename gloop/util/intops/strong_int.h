...
//
// Custom validators cannot return a result, so they can only validate via
// static_assert or side effects (e.g. LOG(FATAL)).
//
...
// strengthened).
//
// TODO: If we made these return the result of the operation, we could
...
// TODO: We should make a return type that is required+meaningful
//     instead of forcing the user to LOG(FATAL).
struct NullStrongIntValidator {
  ...
  // Verify -value.
  template <typename T>
  static constexpr void ValidateNegate(T /*value*/) {}
  // Verify ~value;
  template <typename T>
  static constexpr void ValidateBitNot(T /*value*/) {}
  // Verify lhs + rhs.
  template <typename T>
  ...
  // Verify lhs & rhs.
  template <typename T>
  static constexpr void ValidateBitAnd(T /*lhs*/, T /*rhs*/) {}
  // Verify lhs | rhs.
  template <typename T>
  static constexpr void ValidateBitOr(T /*lhs*/, T /*rhs*/) {}
  // Verify lhs ^ rhs.
  template <typename T>
  static constexpr void ValidateBitXor(T /*lhs*/, T /*rhs*/) {}
};
...
namespace strong_int_internal {

// Check that the result of a validator method call is void. We don't want a
// custom validator to erroneously return a value that is then ignored.
template <typename T>
constexpr void AssertIsVoid() {
  static_assert(std::is_same_v<T, void>,
                "StrongInt validator methods must have void return type.");
}

}  // namespace strong_int_internal

// Invokes a validator method while static_asserting that the expression
// evaluates to void.
#define STRONG_INT_CALL_VALIDATOR(...)                   \
  (strong_int_internal::AssertIsVoid<                    \
       decltype(ValidatorType::template __VA_ARGS__)>(), \
   ValidatorType::template __VA_ARGS__)

// Holds a supported integer value (of type NativeType) and behaves as
...
class StrongInt {
 public:
  ...
  constexpr StrongInt()
      : value_(
            (STRONG_INT_CALL_VALIDATOR(ValidateInit<ValueType>(NativeType())),
             NativeType())) {}
  ...
  StrongInt& operator++() {  // ++x
    STRONG_INT_CALL_VALIDATOR(ValidateAdd<ValueType>(value_, ValueType(1)));
    ++value_;
    return *this;
  }
  ...
  StrongInt& operator--() {  // --x
    STRONG_INT_CALL_VALIDATOR(
        ValidateSubtract<ValueType>(value_, ValueType(1)));
    --value_;
    return *this;
  }
  StrongInt operator--(int postfix_flag) {  // x--
  ...
  StrongInt& operator+=(StrongInt arg) {
    STRONG_INT_CALL_VALIDATOR(ValidateAdd<ValueType>(value_, arg.value()));
    value_ += arg.value();
    return *this;
  }
  StrongInt& operator-=(StrongInt arg) {
    STRONG_INT_CALL_VALIDATOR(ValidateSubtract<ValueType>(value_, arg.value()));
    value_ -= arg.value();
    return *this;
  }
  ...
  StrongInt& operator<<=(int64_t arg) {  // NOLINT(whitespace/operators)
    STRONG_INT_CALL_VALIDATOR(ValidateLeftShift<ValueType>(value_, arg));
    value_ <<= arg;
    return *this;
  }
  StrongInt& operator>>=(int64_t arg) {  // NOLINT(whitespace/operators)
    STRONG_INT_CALL_VALIDATOR(ValidateRightShift<ValueType>(value_, arg));
    value_ >>= arg;
    return *this;
  }
  StrongInt& operator&=(StrongInt arg) {
    STRONG_INT_CALL_VALIDATOR(ValidateBitAnd<ValueType>(value_, arg.value()));
    value_ &= arg.value();
    return *this;
  }
  StrongInt& operator|=(StrongInt arg) {
    STRONG_INT_CALL_VALIDATOR(ValidateBitOr<ValueType>(value_, arg.value()));
    value_ |= arg.value();
    return *this;
  }
  StrongInt& operator^=(StrongInt arg) {
    STRONG_INT_CALL_VALIDATOR(ValidateBitXor<ValueType>(value_, arg.value()));
    value_ ^= arg.value();
    return *this;
  }
...
bool AbslParseFlag(absl::string_view text,
  ...
  // TODO: We shouldn't crash in flag parsing.
...
#define STRONG_INT_VS_STRONG_INT_BINARY_OP(op, validator)                 \
  ...
  constexpr StrongInt<TagType, ValueType, ValidatorType> operator op(     \
    ...
    return (STRONG_INT_CALL_VALIDATOR(                                    \
                validator<ValueType>(lhs.value(), rhs.value())),          \
...
class StrongIntRange {
 public:
  ...
  explicit StrongIntRange(IntType end)
      : begin_(IntType(0)), end_(std::max(IntType(0), end)) {}
  // Loops from begin up to (but not including) end.
  StrongIntRange(IntType begin, IntType end)
      : begin_(begin), end_(std::max(begin, end)) {}
...
// validation of under/overflow situations. An optional third argument can be
// provided to specify a custom validator.
//
...
#endif  // THIRD_PARTY_GLOOP_UTIL_INTOPS_STRONG_INT_H_
