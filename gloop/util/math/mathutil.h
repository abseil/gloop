...
class MathUtil {
 public:
  ...
  [[nodiscard]] ABSL_DEPRECATE_AND_INLINE() static const T& Clamp(
      const T& low, const T& high, const T& value) {
    return std::clamp(value, low, high);
  }

  template <typename T>
  [[nodiscard]] ABSL_DEPRECATE_AND_INLINE() static const T& Clamp(
      const absl::type_identity_t<T>& low, const absl::type_identity_t<T>& high,
