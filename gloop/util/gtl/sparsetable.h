...
// --------------------------------------------------------------------------
// Sponsored Link: You might also be interested in SparseArray<T>,
// defined in sparse_array.h.
//
//   sparsetable<T> provides a space-efficient representation for
//   mostly static sparse arrays.  It allows iteration in time
//   O(# of initialized elements) instead of O(maximum array index).
//   Insertion and deletion force reallocations, so they can be slow.
//   For an array of maximum size M with N elements inserted, the
//   space requirement is approximately N*sizeof(T) + (M-N)*(1-2 bits):
//   uninitialized elements take up very little space.
//
//   SparseArray<T> provides a time-efficient representation for
//   frequently-changing arrays.  It too allows iteration in time
//   O(# of initialized elements) instead of O(maximum array index).
//   Insertion, deletion, and clearing the array are constant time and
//   do not reallocate.  (There is a separate resize() method to change
//   the maximum array index.)
//   For an array of maximum size M, the space requirement is
//   M*(sizeof(T)+2*sizeof(int)): uninitialized elements take up the same
//   space as regular elements, and there are 2 words per element
//   space overhead compared to a regular array.
// --------------------------------------------------------------------------
//
...
//
// Alloc:      Allocator to use to allocate memory.  std::allocator
//
// --- Model of
...
//                                            if read completes successfully
...
#ifndef THIRD_PARTY_GLOOP_UTIL_GTL_SPARSETABLE_H_
#define THIRD_PARTY_GLOOP_UTIL_GTL_SPARSETABLE_H_
...
#include <cstdint>
#include <cstdio>    // to read/write tables
...
/* MOE:begin_strip */
// The glibc htons is buggy -- it uses 'volatile asm' in optizmized
// mode, which apparently doesn't work well with all GCC2 compilers.
// Whatever, we just define our own.
inline u_int16_t my_htons(u_int16_t x) {
  u_int16_t retval = 0;
  u_int16_t* dummy = &retval;  // this is just to be extra-safe
  unsigned char* buffer = reinterpret_cast<unsigned char*>(dummy);
  buffer[0] = x >> 8;
  buffer[1] = x & 255;
  return retval;
}
inline u_int16_t my_ntohs(u_int16_t x) { return my_htons(x); }

inline uint32_t my_htonl(uint32_t x) {
  uint32_t retval = 0;
  uint32_t* dummy = &retval;  // this is just to be extra-safe
  unsigned char* buffer = reinterpret_cast<unsigned char*>(dummy);
  buffer[0] = x >> 24;
  buffer[1] = (x >> 16) & 255;
  buffer[2] = (x >> 8) & 255;
...
inline uint32_t my_ntohl(uint32_t x) { return my_htonl(x); }
/* MOE:end_strip */
...
class table_iterator {
 public:
  ...
  table_iterator() : table(nullptr), pos(0) {}
...
class const_table_iterator {
 public:
  ...
  const_table_iterator(const tabletype* tbl, size_type p)
      : table(tbl), pos(p) {}
  // The default constructor, used when I define vars of type table::iterator
  const_table_iterator() : table(nullptr), pos(0) {}
  // The copy constructor, for when I say table::iterator foo = tbl.begin()
...
#define PUT_(take_from, offset)                                                \
  do {                                                                         \
    if (putc(static_cast<int>(                                                 \
                 ((take_from) >> ((offset) % (sizeof(take_from) * 8))) % 256), \
             fp) == std::char_traits<char>::eof())                             \
      return false;                                                            \
  } while (0)

#define GET_(add_to, offset)                                                \
  do {                                                                      \
    if ((x = getc(fp)) == std::char_traits<char>::eof())                    \
      return false;                                                         \
    else                                                                    \
      add_to |=                                                             \
...
class sparsegroup {
 ...
 private:
  ...
  static size_type modbit(size_type i) {
    return static_cast<size_type>(1 << (i & 7));
  }
  ...
  pointer allocate_group(size_type n) {
    ...
    /* MOE:begin_strip */
    CHECK(retval != nullptr) << "failed to allocate " << n << " groups";
 ...
 public:
  ...
  explicit sparsegroup(const allocator_type& a)
      : group(nullptr), settings(allocator_type(a)) {
    memset(bitmap, 0, sizeof(bitmap));
  }
  sparsegroup(const sparsegroup& x) : group(nullptr), settings(x.settings) {
  ...
  // TODO: Make this exception safe. Handle exceptions in value_type's
  // copy constructor.
  ...
  void swap(sparsegroup& x) noexcept {
 ...
 private:
  // Create space at group[offset].
  void set_aux(size_type offset) {
 ...
 public:
  ...
  // TODO: Make this exception safe: handle exceptions from
 ...
 private:
  // Shrink the array.
  void erase_aux(size_type offset) {
 ...
 public:
  ...
  ABSL_MUST_USE_RESULT bool write_metadata(FILE* fp) const {
  ...
  ABSL_MUST_USE_RESULT bool read_metadata(FILE* fp) {
    clear();
  ...
  ABSL_MUST_USE_RESULT bool write_nopointer_data(FILE* fp) const {
  ...
  ABSL_MUST_USE_RESULT bool read_nopointer_data(FILE* fp) {
 ...
 private:
  ...
  class Settings : public allocator_type {
   public:
    Settings(const allocator_type& a, u_int16_t n = 0)
        : allocator_type(a), num_buckets(n) {}
    Settings(const Settings& s)
        : allocator_type(s), num_buckets(s.num_buckets) {}
...
inline void swap(sparsegroup<T, GROUP_SIZE, Alloc>& x,
                 sparsegroup<T, GROUP_SIZE, Alloc>& y) noexcept {
  x.swap(y);
}
...
class sparsetable {
 private:
  using vector_alloc = typename std::allocator_traits<
      Alloc>::template rebind_alloc<sparsegroup<T, GROUP_SIZE, Alloc>>;
 ...
 public:
  ...
  typedef two_d_iterator<std::vector<
      sparsegroup<value_type, GROUP_SIZE, allocator_type>, vector_alloc>>
      nonempty_iterator;
 ...
 public:
  ...
  // Many STL algorithms use swap instead of copy constructors
  void swap(sparsetable& x) noexcept {
  ...
  size_type max_size() const {
    return std::allocator_traits<Settings>::max_size(settings);
  }
  ...
  // TODO: make protected + friend element_adaptor
  reference mutating_get(size_type i) {  // fills bucket i before getting
    assert(i < settings.table_size);
 ...
 private:
  ...
  /* MOE:begin_strip */
  typedef uint32_t MagicNumberType;
  /* MOE:end_strip_and_replace
  typedef unsigned long MagicNumberType;
  */
  static constexpr MagicNumberType MAGIC_NUMBER = 0x24687531;
 ...
 public:
  ...
  bool operator==(const sparsetable& x) const {
    return settings.table_size == x.settings.table_size &&
           settings.num_buckets == x.settings.num_buckets && groups == x.groups;
  }
...
inline void swap(sparsetable<T, GROUP_SIZE, Alloc>& x,
                 sparsetable<T, GROUP_SIZE, Alloc>& y) noexcept {
  x.swap(y);
}

template <class T, u_int16_t GROUP_SIZE, class Alloc>
inline std::ostream& operator<<(
    std::ostream& out, const sparsetable<T, GROUP_SIZE, Alloc>& table) {
  out << "{";
  int displayed = 0;
...
// Old names for <link>:
using ::gtl::const_table_iterator;        // NOLINT
using ::gtl::const_two_d_iterator;        // NOLINT
using ::gtl::destructive_two_d_iterator;  // NOLINT
using ::gtl::my_htonl;                    // NOLINT
using ::gtl::my_htons;                    // NOLINT
using ::gtl::my_ntohl;                    // NOLINT
using ::gtl::my_ntohs;                    // NOLINT
using ::gtl::sparsegroup;                 // NOLINT
using ::gtl::sparsetable;                 // NOLINT
using ::gtl::table_element_adaptor;       // NOLINT
using ::gtl::table_iterator;              // NOLINT
using ::gtl::two_d_iterator;              // NOLINT

#endif  // THIRD_PARTY_GLOOP_UTIL_GTL_SPARSETABLE_H_
