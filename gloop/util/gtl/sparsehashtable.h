...
#ifndef THIRD_PARTY_GLOOP_UTIL_GTL_SPARSEHASHTABLE_H_
#define THIRD_PARTY_GLOOP_UTIL_GTL_SPARSEHASHTABLE_H_
...
#include "gloop/util/gtl/hashtable_common.h"
#include "gloop/util/gtl/sparsetable.h"  // IWYU pragma: export
#include "gloop/util/hash/hash.h"
/* MOE:begin_strip */

namespace gtl {

// Legacy hash function implementation.
// cl/245119902 changed the implementation of the default hash functions.
// Unfortunately, the hash function is hardcoded in the wire format of sparse
// hashtables. To avoid breaking the wire format, we provide the legacy hash
// function implementation here for the types that were using the wire format.
// <link>
template <typename T>
struct SparseHashTableLegacyHash {
  static_assert(std::is_integral_v<T>, "Only supported for integral types.");
  size_t operator()(T t) const { return static_cast<size_t>(t); }
};

// Cloned from util/hash/hash.h
template <>
struct SparseHashTableLegacyHash<std::string> {
  size_t operator()(const std::string& t) const {
    return static_cast<size_t>(HashTo32(t));
  }
};

// Cloned from util/hash/hash.h
template <typename T, std::size_t N>
struct SparseHashTableLegacyHash<std::array<T, N>> {
  size_t operator()(const std::array<T, N>& t) const {
    return HashTuple(t, 71);
  }

 private:
  static size_t Mix(size_t new_hash, size_t accu) {
  ...
  struct HashTupleImpl {
    size_t operator()(const Tup& t, size_t seed) const {
      return Mix(SparseHashTableLegacyHash<T>()(t[I]),
                 HashTupleImpl<Tup, I + 1>{}(t, seed));
    }
  };
  template <class Tup>
  struct HashTupleImpl<Tup, N> {
    size_t operator()(const Tup&, size_t seed) const { return seed; }
  };

  template <typename Tup>
  static size_t HashTuple(const Tup& t, size_t seed) {
    return 0xc6a4a7935bd1e995ULL * HashTupleImpl<Tup>{}(t, seed);
  }
};

namespace internal_sparsetable {

template <typename Hash, typename = void>
struct SelectDefaultHash {
  using type = Hash;
};

template <typename T>
struct SelectDefaultHash<absl::Hash<T>,
                         std::enable_if_t<std::is_integral_v<T>>> {
  using type = gtl::SparseHashTableLegacyHash<T>;
};

template <typename T, size_t N>
struct SelectDefaultHash<absl::Hash<std::array<T, N>>> {
  using type = gtl::SparseHashTableLegacyHash<std::array<T, N>>;
};

template <>
struct SelectDefaultHash<absl::Hash<std::string>> {
  using type = gtl::SparseHashTableLegacyHash<std::string>;
};
}  // namespace internal_sparsetable
...
struct sparse_hashtable_iterator {
 ...
 public:
  ...
  sparse_hashtable_iterator() = default;  // not ever used internally
...
struct sparse_hashtable_const_iterator {
 ...
 public:
  ...
  sparse_hashtable_const_iterator() = default;  // never used internally
...
struct sparse_hashtable_destructive_iterator {
 ...
 public:
  ...
  sparse_hashtable_destructive_iterator() = default;  // never used internally
  // The default destructor is fine; we don't define one
  // The default operator= is fine; we don't define one
...
class sparse_hashtable {
 ...
 private:
  ...
  // std::pair<const X, Y>).  We use explicit destructor invocation and
 ...
 private:
  void check_use_deleted(const char* caller) {
    /* MOE:begin_strip */
    if (!settings.use_deleted()) {
      LOG(DFATAL) << caller << " is dangerous if set_deleted_key() wasn't"
                  << " called first.  See the comment for set_deleted_key().";
    }
    /* MOE:end_strip_and_replace
 ...
 private:
  ...
  // TODO: take a delta so we can take into account inserts
  ...
  bool resize_delta(size_type delta) {
    ...
    if (table.num_nonempty() >=
      ...
      /* MOE:begin_strip */
      LOG(FATAL) << "Growing hashtable overflows size_type";
      /* MOE:end_strip_and_replace
    ...
    assert(settings.shrink_factor() > 0 || resize_to >= bucket_count());
 ...
 public:
  ...
  void swap(sparse_hashtable& ht) noexcept {
 ...
 private:
  ...
  iterator insert_at(const_reference obj, size_type pos) {
    if (size() >= max_size()) {
      /* MOE:begin_strip */
      LOG(FATAL) << "size will exceed max_size";
      /* MOE:end_strip_and_replace
  ...
  void insert(ForwardIterator f, ForwardIterator l, std::forward_iterator_tag) {
    ...
    if (dist >= (std::numeric_limits<size_type>::max)()) {
      /* MOE:begin_strip */
      LOG(FATAL) << "distance of " << dist << " overflows size_type";
      /* MOE:end_strip_and_replace
 ...
 public:
  ...
  ABSL_MUST_USE_RESULT bool read_metadata(INPUT* fp) {
  ...
  // Only meaningful if value_type is a POD.
  template <typename INPUT>
  ABSL_MUST_USE_RESULT bool read_nopointer_data(INPUT* fp) {
  ...
  class KeyInfo : public ExtractKey, public SetKey, public EqualKey {
   public:
    KeyInfo(const ExtractKey& ek, const SetKey& sk, const EqualKey& eq)
        : ExtractKey(ek), SetKey(sk), EqualKey(eq), delkey() {}
    ...
    // TODO: make a pointer, and get rid of use_deleted (benchmark!)
    std::remove_const_t<key_type> delkey;
  };
...
inline void swap(sparse_hashtable<V, K, HF, ExK, SetK, EqK, A>& x,
                 sparse_hashtable<V, K, HF, ExK, SetK, EqK, A>& y) noexcept {
  x.swap(y);
}
...
// Old names for <link>:
using ::gtl::DEFAULT_GROUP_SIZE;
using ::gtl::sparse_hashtable;                       // NOLINT
using ::gtl::sparse_hashtable_const_iterator;        // NOLINT
using ::gtl::sparse_hashtable_destructive_iterator;  // NOLINT
using ::gtl::sparse_hashtable_iterator;              // NOLINT
/* MOE:end_strip_and_replace
_END_GOOGLE_NAMESPACE_
*/

#endif  // THIRD_PARTY_GLOOP_UTIL_GTL_SPARSEHASHTABLE_H_
