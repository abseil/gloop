 ...
 public:
  ...
  UniqueArray(T* ptr, size_t size)
    ...
    if (ptr == nullptr) {
      ...
      ABSL_HARDENING_ASSERT(size_le_than_cookie<deleter_type>(ptr, size));
    }
  }
  ...
  UniqueArray(std::unique_ptr<T[], deleter_type> ptr, size_t size)
    ...
    if (data_ == nullptr) {
      ...
      ABSL_HARDENING_ASSERT(
          size_le_than_cookie<deleter_type>(data_.get(), size));
    }
  }
  ...
  constexpr bool size_le_than_cookie(ArrayType const* ptr, size_t size) {
    if constexpr (is_default_deleter<ArrayDeleter>::value) {
      if constexpr (has_array_cookie<ArrayType>::value) {
        ...
        return size <= cookie;
#ifndef NDEBUG
      } else if (auto heap_size = tcmalloc::MallocExtension::GetAllocatedSize(
                     const_cast<std::remove_volatile_t<ArrayType>*>(ptr));
                 heap_size.has_value()) {
        return size * sizeof(ArrayType) <= heap_size.value();
#endif  // NDEBUG
      }
    }
...
#endif  // THIRD_PARTY_GLOOP_UTIL_GTL_UNIQUE_ARRAY_H_
