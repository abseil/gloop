...
using ManyFields =
    gtl::internal_extend::ManyFields<gtl::DebugPrintingExtension>;
using MultipleExtends =
    gtl::internal_extend::MultipleExtends<gtl::DebugPrintingExtension>;
using Nested = gtl::internal_extend::Nested<gtl::DebugPrintingExtension>;
using OneField = gtl::internal_extend::OneField<gtl::DebugPrintingExtension>;
using Streamer = gtl::internal_extend::Streamer;
template <typename T>
using Template = gtl::internal_extend::Template<T, gtl::DebugPrintingExtension>;

template <typename>
using DebugPrinting = testing::Test;
using Types = testing::Types<Streamer, gtl::internal_extend::StrFormatter,
                             gtl::internal_extend::StrCatter>;
TYPED_TEST_SUITE(DebugPrinting, Types);

TYPED_TEST(DebugPrinting, OneField) {
...
TYPED_TEST(DebugPrinting, ManyFields) {
  if constexpr (!MatchesUsingRE2()) {
    GTEST_SKIP() << "Test assumes RE2 syntax, but RE2 is not being used";
  }
...
TYPED_TEST(DebugPrinting, Nested) {
  if constexpr (!MatchesUsingRE2()) {
    GTEST_SKIP() << "Test assumes RE2 syntax, but RE2 is not being used";
  }
...
TYPED_TEST(DebugPrinting, Template) {
  if constexpr (!MatchesUsingRE2()) {
    GTEST_SKIP() << "Test assumes RE2 syntax, but RE2 is not being used";
  }
...
TYPED_TEST(DebugPrinting, MultipleExtends) {
  if constexpr (!MatchesUsingRE2()) {
    GTEST_SKIP() << "Test assumes RE2 syntax, but RE2 is not being used";
  }
...
TYPED_TEST(DebugPrinting, TemplateTemplate) {
  if constexpr (!MatchesUsingRE2()) {
    GTEST_SKIP() << "Test assumes RE2 syntax, but RE2 is not being used";
  }
...
class Class : public gtl::Extend<Class, 1>::With<gtl::DebugPrintingExtension> {
 public:
...
TYPED_TEST(DebugPrinting, Class) {
  if constexpr (!MatchesUsingRE2()) {
    GTEST_SKIP() << "Test assumes RE2 syntax, but RE2 is not being used";
  }
...
struct Complicated
    : gtl::Extend<Complicated, 1>::With<gtl::DebugPrintingExtension> {
  std::unique_ptr<int> n;
};

TYPED_TEST(DebugPrinting, Complicated) {
  if constexpr (!MatchesUsingRE2()) {
    GTEST_SKIP() << "Test assumes RE2 syntax, but RE2 is not being used";
  }
...
struct EmptyStruct
    : gtl::Extend<EmptyStruct>::With<gtl::DebugPrintingExtension> {};

TYPED_TEST(DebugPrinting, EmptyStruct) {
  if constexpr (!MatchesUsingRE2()) {
    GTEST_SKIP() << "Test assumes RE2 syntax, but RE2 is not being used";
  }
