...
TYPED_TEST(CircularBufferTest, PushFront) {
  ...
  // Test at()
  EXPECT_EQ(4, cb.at(0));
  EXPECT_EQ(&cb[0], &cb.at(0));
  EXPECT_EQ(3, cb.at(1));
  EXPECT_EQ(&cb[1], &cb.at(1));
  EXPECT_EQ(2, cb.at(2));
...
TEST(CircularBufferSizeOfTest, InlineCapacity) {
  EXPECT_EQ(sizeof(CircularBuffer<int, 4>) - sizeof(CircularBuffer<int>),
            4 * sizeof(int));
}
