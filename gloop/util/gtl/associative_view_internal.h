...
#ifndef THIRD_PARTY_GLOOP_UTIL_GTL_ASSOCIATIVE_VIEW_INTERNAL_H_
#define THIRD_PARTY_GLOOP_UTIL_GTL_ASSOCIATIVE_VIEW_INTERNAL_H_
...
inline constexpr bool kHasSubscript =
    gtl::Requires<C, const K>([](auto&& c, auto&& k) -> decltype(c[k]) {});
...
template <typename C, typename K = typename C::key_type>
inline constexpr bool kHasAt = gtl::Requires<const C, const K>(
    [](auto&& c, auto&& k) -> decltype(c.at(k)) {});

struct NoneSuch {};

template <typename C>
inline constexpr bool kHasHeterogeneousLookup =
    gtl::Requires<const C>([](auto&& c) -> decltype(c.find(NoneSuch{})) {});
...
inline constexpr bool kHasContainsMethod = gtl::Requires<const C, const K>(
...
const mapped_type& AtFunction(const View& v, const Key& key) {
  ...
  if (ABSL_PREDICT_FALSE(it == v.end())) {
    absl::ThrowStdOutOfRange("MapView::at failed bounds check");
  }
  return it->second;
}
...
//       gtl::AlsoSupportsLookupWith<absl::string_view>;
//   using StringSetView = gtl::SetView<std::string, WithStringViewLookup>;
//
...
class AssociateView {
  ...
  static_assert(!std::is_same_v<ExtraLookupTypes, ExtraLookupTypes>,
                ...
                "instantiation of gtl::AlsoSupportsLookupWith");
};
...
// Note: key types may have type modifiers (i.e. gtl::SetView<const int>), and
// we remove them when interacting with the internal virtual table. Consider:
//    gtl::SetView<const int> my_view = ...;
...
class AssociateView<KeyType, ValueType, AlsoSupportsLookupWith<Keys...>>
 ...
 protected:
  ...
  // container.
  constexpr AssociateView() = default;
  ...
  //   and `key_type` must be constructible from every type in
...
class AssociateView<K, V, AlsoSupportsLookupWith<Keys...>>::iterator {
 ...
 private:
  ...
  iterator(const C& c, It&& it, IteratorAdapter)
      // NOTE: `value_` can point to a dereference of any copy of
      // `it` (see comments on `value_`).
    ...
    static_assert(
        ...
        "only forward iterators are supported, see b/347662717");
    assert(it != c.end());
    if constexpr (IsInline<Iterator>()) {
  ...
  // c_->end(). We can use a pointer here because the underlying iterator is a
  // forward iterator, which guarantees that `reference` is `value_type&` or
  // `const value_type&`. Furthermore, forward iterators guarantee that "If `a`
  // and `b` compare equal [...] then [...] `*a` and `*b` are references bound
  // to the same object", so the pointed-to object is always valid and
  // independent of the underlying iterator object.
...
#endif  // THIRD_PARTY_GLOOP_UTIL_GTL_ASSOCIATIVE_VIEW_INTERNAL_H_
