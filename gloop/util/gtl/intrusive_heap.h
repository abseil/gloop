...
// named "heap" (by default) of type gtl::IntrusiveHeapLink, which the
...
#ifndef THIRD_PARTY_GLOOP_UTIL_GTL_INTRUSIVE_HEAP_H_
#define THIRD_PARTY_GLOOP_UTIL_GTL_INTRUSIVE_HEAP_H_
...
// gtl::IntrusiveHeap<T, PtrCompare, LinkAccess, Alloc>
//
...
//   gtl::IntrusiveHeapLink data member (usually called 'heap').
//
...
//      '::gtl::IntrusiveHeapLink Get(const T* elem)' and
//      void Set(T* elem, ::gtl::IntrusiveHeapLink link)'.
...
class IntrusiveHeap {
 private:
  typedef std::vector<T*, Alloc> heap_type;
 ...
 public:
  ...
  // There is NO guarantee on the ordering of items returned.
  // In particular, the `top` element is not guaranteed to be at the
  // beginning (or end) of the range.
  const_iterator begin() const { return heap().begin(); }
  const_iterator end() const { return heap().end(); }
  const_iterator cbegin() const { return heap().cbegin(); }
  const_iterator cend() const { return heap().cend(); }
...
#endif  // THIRD_PARTY_GLOOP_UTIL_GTL_INTRUSIVE_HEAP_H_
