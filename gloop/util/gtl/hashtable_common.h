...
#ifndef THIRD_PARTY_GLOOP_UTIL_GTL_HASHTABLE_COMMON_H_
#define THIRD_PARTY_GLOOP_UTIL_GTL_HASHTABLE_COMMON_H_
  ...
  size_type min_buckets(size_type num_elts, size_type min_buckets_wanted) {
    ...
    while (sz < min_buckets_wanted ||
      ...
      if (static_cast<size_type>(sz * 2) < sz) {
        /* MOE:begin_strip */
        LOG(FATAL) << "min_buckets: doubling of " << sz
                   << " overflows size_type. " << " initial=" << HT_MIN_BUCKETS
                   << " num_elts=" << num_elts << " enlarge=" << enlarge;
        /* MOE:end_strip_and_replace
  ...
  class hash_munger<HashKey*> {
   public:
    static size_t MungedHash(size_t hash) {
      // TODO: consider rotating instead:
...
// Old names for <link>:
using ::gtl::sh_hashtable_settings;  // NOLINT
using ::gtl::sh_is_transparent;      // NOLINT

/* MOE:end_strip */

#endif  // THIRD_PARTY_GLOOP_UTIL_GTL_HASHTABLE_COMMON_H_
