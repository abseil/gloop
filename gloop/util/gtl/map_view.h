...
#ifndef THIRD_PARTY_GLOOP_UTIL_GTL_MAP_VIEW_H_
#define THIRD_PARTY_GLOOP_UTIL_GTL_MAP_VIEW_H_
...
// A type that is provided to `gtl::MapView` as `ExtraLookupTypes` parameter
...
//   template <typename T>
//   using ThreadMapView = gtl::MapView<std::thread, T, WithThreadIdLookup>;
//
...
//
// By default, `gtl::MapView` sets ExtraLookupTypes to
...
// differently from `std::pair<const K, V>`. Notable example of such map is
// `gtl::string_hash_map`.
template <typename K, typename V,
          typename ExtraLookupTypes =
              internal_associative_view::DefaultExtraLookupForKey<K>>
class ABSL_ATTRIBUTE_VIEW MapView
...
#endif  // THIRD_PARTY_GLOOP_UTIL_GTL_MAP_VIEW_H_
