...
// Pins which overload each element-type combination below selects, so the
// comments in the tests cannot rot.
static_assert(
    gtl::c_mem_internal::kCanUseCCopy<std::vector<int>&, std::vector<int>>);
static_assert(
    gtl::c_mem_internal::kCanUseCCopyN<std::vector<char>&, std::vector<char>>);
static_assert(
...
TEST(CMemcpyTest, CopiesPartialElements) {
...
TEST(CMemcpyTest, WorksForContainersOfSingleByteType) {
  ...
  c_memcpy(str_dest, absl::string_view("ab"), 2);
...
TEST(CMemcpyTest, WorksForSpans) {
  ...
  c_memcpy(lvalue_span, absl::MakeConstSpan(v4).subspan(0, 2));
...
TEST(CMemcpyDeathTest, CrashesOnOutOfBoundsWrite) {
  ...
  if (IsHardened()) {
    EXPECT_DEATH(c_memcpy(dest, src, 4 * sizeof(int)), "");  // memcpy
    EXPECT_DEATH(c_memcpy(dest, src), "");                   // absl::c_copy
    EXPECT_DEATH(c_memcpy(dest_diff_type, src), "");         // memcpy
  }
#endif
}
...
#if GTEST_HAS_DEATH_TEST
  if (IsHardened()) {
    EXPECT_DEATH(c_memcpy(dest, src, 4), "");            // absl::c_copy_n
    EXPECT_DEATH(c_memcpy(dest, src), "");               // absl::c_copy
    EXPECT_DEATH(c_memcpy(dest_diff_type, src, 4), "");  // memcpy
    EXPECT_DEATH(c_memcpy(dest_diff_type, src), "");     // memcpy
  }
#endif
}
...
#if GTEST_HAS_DEATH_TEST
  if (IsHardened()) {
    EXPECT_DEATH(c_memcpy(dest, src, 4), "");                        // c_copy_n
    EXPECT_DEATH(c_memcpy(dest_diff_type, src, 4), "");              // memcpy
    EXPECT_DEATH(c_memcpy(dest_int, src_int, 4 * sizeof(int)), "");  // memcpy
  }
#endif
}
