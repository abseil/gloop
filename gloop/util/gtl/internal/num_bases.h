...
#ifndef THIRD_PARTY_GLOOP_UTIL_GTL_INTERNAL_NUM_BASES_H_
#define THIRD_PARTY_GLOOP_UTIL_GTL_INTERNAL_NUM_BASES_H_
...
constexpr std::optional<int> NumBasesImpl(std::index_sequence<I...>) {
  if constexpr (BraceInitializableWith<T, OnlyBases<T>, OnlyBases<T>,
                                       decltype(I, Anything{})...>::value) {
    // The first two initializers look like bases.
    // The second might be a member or a base, so we give up.
    return std::nullopt;
  } else if constexpr (BraceInitializableWith<T, OnlyBases<T>, Anything,
                                              decltype(I,
                                                       Anything{})...>::value) {
...
#endif  // THIRD_PARTY_GLOOP_UTIL_GTL_INTERNAL_NUM_BASES_H_
