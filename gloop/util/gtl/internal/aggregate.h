...
#ifndef THIRD_PARTY_GLOOP_UTIL_GTL_INTERNAL_AGGREGATE_H_
#define THIRD_PARTY_GLOOP_UTIL_GTL_INTERNAL_AGGREGATE_H_
...
constexpr int NumFields() {
  if constexpr (std::is_empty_v<T>) {
    ...
    if constexpr (kNumBases == -1) {
      constexpr auto num_bases = internal_aggregate::NumBases<T>();
      ...
      return internal_aggregate::NumInitializers<T>() - num_bases.value();
    } else {
      return internal_aggregate::NumInitializers<T>() - kNumBases;
    }
  } else {
    return -1;
  }
...
struct FieldGetter {
  ...
  constexpr static auto Unpack(T&& t) {
    static_assert(
        ...
        "base class or 1 empty base class (gtl::Extend). You can "
    ...
    if constexpr (kFieldCount == -1) {
      ...
      auto&& [f0,  // Did you forget `friend gtl::EnableExtensions`?
              f1] = t;
      return std::forward_as_tuple(CorrectQualifiers<T, decltype(f0)>(f0),
...
#endif  // THIRD_PARTY_GLOOP_UTIL_GTL_INTERNAL_AGGREGATE_H_
