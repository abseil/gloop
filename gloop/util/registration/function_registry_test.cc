...
#include "gloop/util/registration/function_registry.h"
...
TEST(FunctionRegistryTest, RegisterSucceeds) {
  ...
  EXPECT_TRUE(registry.Register(
      "foo", [] { return 1; }, absl::SourceLocation::current()));
  RegisterOrDie(
      &registry, "bar", [] { return 2; }, absl::SourceLocation::current());
}
...
TEST(FunctionRegistryTest, Unregister) {
  FunctionRegistry<absl::string_view, int()> registry;
  registry.Register(kKey, [] { return 1; }, absl::SourceLocation::current());
...
TEST(FunctionRegistryTest, GetSucceeds) {
  ...
  FunctionRegistry<absl::string_view, int(std::unique_ptr<int>)>  // NOLINT
      registry;
  ASSERT_TRUE(registry.Register(
      kKey, [](std::unique_ptr<int> x) { return *x; },
...
TEST(FunctionRegistryTest, GetAll) {
  ...
  ASSERT_TRUE(registry.Register(
      "foo", [] { return 1; }, absl::SourceLocation::current()));
  ASSERT_TRUE(registry.Register(
      "bar", [] { return 2; }, absl::SourceLocation::current()));
...
TEST(FunctionRegistryTest, StatefulFunctor) {
  ...
  FunctionRegistry<absl::string_view, int()> registry;
  ASSERT_TRUE(
      registry.Register(kKey, Counter{}, absl::SourceLocation::current()));
...
TEST(FunctionRegistryTest, UnsetFunction) {
  ...
  ASSERT_TRUE(
      registry.Register(kKey, original_fn, absl::SourceLocation::current()));
...
TEST(FunctionRegistryTest, ScopedRegistrationWithHeterogeneousLookup) {
  FunctionRegistry<std::string, int()> registry;
  EXPECT_FALSE(registry.Get("foo"));
  {
    absl::string_view key = "foo";
    ScopedRegistration registration(registry, key, [] { return 1; });
    EXPECT_TRUE(registry.Get("foo"));
  }
  // "foo" was unregistered at the end of the scope
  EXPECT_FALSE(registry.Get("foo"));
