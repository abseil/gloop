...
//
// FooRegistry& GetGlobalFooRegistry() {
//   static absl::NoDestructor<FooRegistry> r;  // https://abseil.io/tips/110.
//   return *r;
// }
//
// === client.h ==========
...
// that includes the registration (<link>).
//
...
// global object in https://abseil.io/tips/110.
//
// === Registering functions ==================================================
//
// To register a function, call Register with a key and function. The key must
// be copyable and hashable using <link>, and the function may be any
...
bool RegisterOrDie(Registry* registry, const Key& key,
                   typename Registry::Function fn,
                   absl::SourceLocation loc = absl::SourceLocation::current()) {
  return RegisterOrDie(*registry, key, std::move(fn), loc);
}
...
class ScopedRegistration {
 public:
  template <typename Key = typename Registry::Key>
  ScopedRegistration(Registry& registry, const Key& key,
                     typename Registry::Function fn)
...
#endif  // THIRD_PARTY_GLOOP_UTIL_REGISTRATION_FUNCTION_REGISTRY_H_
