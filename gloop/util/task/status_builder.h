...
#ifndef THIRD_PARTY_GLOOP_UTIL_TASK_STATUS_BUILDER_H_
#define THIRD_PARTY_GLOOP_UTIL_TASK_STATUS_BUILDER_H_
...
class Stream {
 public:
  explicit Stream(std::string& message)
      : message_(&message), absl_stringify_stream_(ostringstream_) {}

  template <typename T>
  friend Stream& operator<<(Stream& stream, const T& t) {
    stream.absl_stringify_stream_ << t;
    *stream.message_ = stream.ostringstream_.str();
    return stream;
  }
...
#endif  // THIRD_PARTY_GLOOP_UTIL_TASK_STATUS_BUILDER_H_
