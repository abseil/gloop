...
TEST(stringtest, safe_strtosize_t_random) {
  absl::InsecureBitGen rng;
  test_random_integer_parse<size_t, safe_strtosize_t>(rng);
  test_random_integer_parse_base<size_t, safe_strtosize_t_base>(rng);
}
...
static_assert(sizeof(size_t) == 8, "expected 64bit arch");
const size_t_test_case strtosize_t_test_cases[] = {
    {"0x3423432448783446", true, 16, int64_t{0x3423432448783446}},
    {"3423432448783446", true, 16, int64_t{0x3423432448783446}},
...
#else
static_assert(sizeof(size_t) == 4, "expected 32bit arch");
const size_t_test_case strtosize_t_test_cases[] = {
    {"0xffffffff", true, 16, std::numeric_limits<uint32_t>::max()},
    {"0x34234324", true, 16, 0x34234324},
    {"34234324", true, 16, 0x34234324},
    ...
    // Out of bounds.
    {"4294967296", false, 0, 0},
    {"0x100000000", false, 0, 0},
    {nullptr, false, 0, 0},
};
#endif

TEST(stringtest, safe_strtosize_t_base) {
  for (int i = 0; strtosize_t_test_cases[i].str != nullptr; ++i) {
    const struct size_t_test_case& e = strtosize_t_test_cases[i];
    size_t value;
    EXPECT_EQ(e.expect_ok, safe_strtosize_t_base(e.str, &value, e.base))
        << "str=\"" << e.str << "\" base=" << e.base;
    if (e.expect_ok) {
      EXPECT_EQ(e.expected, value) << "str=\"" << e.str << "\" base=" << e.base;
    }
...
TEST(stringtest, safe_strtosize_t) {
  for (int i = 0; strtosize_t_test_cases[i].str != nullptr; ++i) {
    const struct size_t_test_case& e = strtosize_t_test_cases[i];
    if (e.base != 0) {  // Skip non-base-10 tests.
      continue;
    ...
    size_t value;
    EXPECT_EQ(e.expect_ok, safe_strtosize_t(e.str, &value))
        << "str=\"" << e.str;
    if (e.expect_ok) {
      EXPECT_EQ(e.expected, value) << "str=\"" << e.str << "\"";
    }
...
TEST(stringtest, safe_strtosize_t_base_length_delimited) {
  for (int i = 0; strtosize_t_test_cases[i].str != nullptr; ++i) {
    const struct size_t_test_case& e = strtosize_t_test_cases[i];
    std::string tmp(e.str);
    tmp.append("12");  // Adds garbage at the end.
