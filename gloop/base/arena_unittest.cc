...
ABSL_FLAG(int32_t, numobjects, 500000, "Number of objects to allocate");
ABSL_FLAG(int32_t, iterations, 2, "Number of iterations to try");
ABSL_FLAG(bool, timing, true, "Do timing tests");
ABSL_FLAG(int32_t, stl_iterations, 1, "Number of iterations on the STL tests.");
...
void TestArena2(UnsafeArena* const arena) {
  ...
  char* s_ten = arena->Realloc(s4, 100, 10);
  char* s_eleven = arena->Realloc(s4, 10, 100);
  char* s12 = arena->Strdup(s9);
  char* s13 = arena->Realloc(s9, sizeof(sshort) - 1, 100000);  // won't fit :-)
  char* s14 = arena->Snprintf(100, "%s", sshort);
  char* s15 = arena->Snprintf(100, "%s", slong);
  char* s16 = arena->Snprintf(100, "%d%s%d", 69, slong, 96);
  ...
  CHECK_EQ(s_ten, s4);  // Realloc-smaller should cause us to realloc in place
  ...
  CHECK_NE(s_eleven, s4);
  CHECK_EQ(0, strncmp(s_eleven, slong, 10));
  CHECK_EQ(0, strcmp(s12, s1));
  CHECK_NE(s12, s13);  // too big to grow-in-place, so we should move
  CHECK_EQ(0, strcmp(s14, sshort));
  CHECK(strlen(s15) == 99 && strncmp(s15, slong, 99) == 0);
  CHECK(strlen(s16) == 99 && s16[0] == '6' && s16[1] == '9' &&
        strncmp(s16 + 2, slong, 97) == 0);
