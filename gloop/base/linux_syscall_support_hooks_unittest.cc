...
// Fake as though the code is being compiled with a sanitizer.
#define LINUX_SYSCALL_ENABLE_HOOKS 1
#define SYS_CPLUSPLUS 1

// Prototypes for LSS_PRE/POST macros under test.
void __sanitizer_syscall_pre_geteuid();
void __sanitizer_syscall_post_geteuid(int res);

#if !defined(__aarch64__) && !defined(__riscv)
// On Aarch64 stat is emulated using newfstatat.
void __sanitizer_syscall_pre_stat(const char* f, struct kernel_stat* b);
void __sanitizer_syscall_post_stat(int res, const char* f,
                                   struct kernel_stat* b);
#define __sanitizer_syscall_pre_newfstatat(...)
#define __sanitizer_syscall_post_newfstatat(res, ...)
#else
void __sanitizer_syscall_pre_newfstatat(int d, const char* f,
                                        struct kernel_stat* b, int flags);
void __sanitizer_syscall_post_newfstatat(int res, int d, const char* f,
                                         struct kernel_stat* b, int flags);
#define __sanitizer_syscall_pre_stat(...)
#define __sanitizer_syscall_post_stat(res, ...)
#endif

// Provide empty definitions for LSS_PRE/POST macros for system calls not under
// test.
...
#define __sanitizer_syscall_pre_epoll_pwait2(...)
...
#define __sanitizer_syscall_pre_execveat(...)
...
#define __sanitizer_syscall_pre_getdents64(...)
...
#define __sanitizer_syscall_pre_rt_sigpending(...)
...
#define __sanitizer_syscall_post_epoll_pwait2(res, ...)
...
#define __sanitizer_syscall_post_execveat(res, ...)
...
#define __sanitizer_syscall_post_getdents64(res, ...)
...
#define __sanitizer_syscall_post_rt_sigpending(res, ...)
...
static struct kernel_stat* hook_b;
static unsigned hook_ino;

#if !defined(__aarch64__) && !defined(__riscv)
void __sanitizer_syscall_pre_stat(const char* f, struct kernel_stat* b) {
  ASSERT_EQ(nullptr, hook_b);
  hook_b = b;
  ASSERT_EQ(nullptr, hook_f);
  hook_f = f;
  hook_res = -42;
}
...
void __sanitizer_syscall_post_stat(int res, const char* f,
  ...
  ASSERT_EQ(hook_b, b);
  hook_b = nullptr;
  ASSERT_EQ(-42, hook_res);
  hook_res = res;
  if (!res) hook_ino = b->st_ino;
}
#else
void __sanitizer_syscall_pre_newfstatat(int d, const char* f,
                                        struct kernel_stat* b, int flags) {
  ASSERT_EQ(d, AT_FDCWD);
  ASSERT_EQ(0, flags);
  ASSERT_EQ(0, hook_b);
  hook_b = b;
  ASSERT_EQ(0, hook_f);
...
TEST(LinuxSyscallSupportHooks, stat) {
  struct kernel_stat kernel_stat;
  // Clear kernel_stat. This is needed for this test to pass under
  // MemorySanitizer due to the fact that it overrides sanitizer syscall
  // handlers for stat().
  memset(&kernel_stat, 0, sizeof(kernel_stat));
  EXPECT_EQ(0, sys_stat("/dev/null", &kernel_stat));
  EXPECT_EQ(0, hook_res);
  EXPECT_EQ(hook_ino, kernel_stat.st_ino);

  int res = sys_stat("/invalid/path", &kernel_stat);
  EXPECT_EQ(-1, res);
  EXPECT_EQ(ENOENT, errno);
  EXPECT_EQ(-ENOENT, hook_res);
}

static int geteuid_pre_cnt;
static int geteuid_post_cnt;
static int geteuid_res;

void __sanitizer_syscall_pre_geteuid() { ++geteuid_pre_cnt; }
...
TEST(LinuxSyscallSupportHooks, geteuid) {
  int res = sys_geteuid();
  EXPECT_EQ(1, geteuid_pre_cnt);
  EXPECT_EQ(1, geteuid_post_cnt);
  EXPECT_EQ(res, geteuid_res);
}
