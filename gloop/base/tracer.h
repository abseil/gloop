...
class Tracer {
 public:
  ...
  // is set.
  absl::Duration lock_contention_delay() const {
    return absl::Nanoseconds(
        lock_contention_delay_ns_.load(std::memory_order_relaxed));
  }
  ...
  void add_lock_contention_delay_safe(absl::Duration duration) {
    lock_contention_delay_ns_.fetch_add(
        static_cast<uint64_t>(absl::ToInt64Nanoseconds(duration)),
 ...
 private:
  ...
  // Stores the lock contention delay in nanoseconds.
  std::atomic<uint64_t> lock_contention_delay_ns_{0};
