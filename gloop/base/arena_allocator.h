...
#ifndef THIRD_PARTY_GLOOP_BASE_ARENA_ALLOCATOR_H_
#define THIRD_PARTY_GLOOP_BASE_ARENA_ALLOCATOR_H_
...
class ArenaAllocator {
 public:
  ...
  T* allocate(size_t n, const void* /*hint*/ = nullptr) {
    ...
    size_t align = kAlignment;
    return reinterpret_cast<T*>(arena_->AllocAligned(req_size, align));
  }
  void deallocate(T* p, size_t n) {
    size_t req_size = n * sizeof(T);
    arena_->Free(p, req_size);
  }

  C* arena() const { return arena_; }

  template <class U>
  ArenaAllocator(const ArenaAllocator<U, C>& other) : arena_(other.arena()) {}

  template <class U>
  bool operator==(const ArenaAllocator<U, C>& other) const {
...
class Gladiator {
 public:
  ...
  void* operator new(const size_t size, const int ignored, T* allocator) {
    if (allocator) {
      void* ret =
          allocator->AllocAligned(1 + size, BaseArena::kDefaultAlignment);
...
// arenas. It enables allocation with base::NewInArea and prevents accidental
// use of the global operator new and delete with derived types.
//
// Brief example (see documentation for base::NewInArena / base::DeleteInArena
// for full details):
//
...
//   MyType* t1 = base::NewInArena<MyType>(&arena);
//
...
class ArenaOnlyGladiator {
 public:
  ...
  // the ignored parameter keeps us from stepping on placement new
  template <class T>
  ABSL_DEPRECATED("use base::NewInArena from base/arena_allocator.h instead")
  void* operator new(const size_t size, const int ignored, T* allocator) {
    assert(allocator);
  ...
  template <class T>
  ABSL_DEPRECATED("use base::DeleteInArena from base/arena_allocator.h instead")
  void operator delete(void* memory, const size_t size, const int ign,
                       T* allocator) {}
...
//
//   UnsafeArena arena(1024);
//   MyType* p = base::NewInArena<MyType>(&arena, 5, 3);
//   MyType* q = base::NewInArena<MyType[]>(&arena, 10);
//
// When finished, you must destroy the objects and return memory as follows:
//
//   base::DeleteInArena(&arena, p);
//   base::DeleteInArena(&arena, q, 10);
//
...
// DEPRECATED: Use base::NewInArena declared above instead.
//
...
// STL containers, etc.
ABSL_DEPRECATED("use base::NewInArena from base/arena_allocator.h instead")
inline void* operator new(size_t size, AllocateInArenaType /* unused */,
                          UnsafeArena* arena) {
  return arena->AllocAligned(size, BaseArena::kDefaultAlignment);
}
...
#endif  // THIRD_PARTY_GLOOP_BASE_ARENA_ALLOCATOR_H_
