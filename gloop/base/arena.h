...
//
// WITH base::NewInArena<T>: Use this to allocate a C++ object of type T, which
//    must inherit from ArenaOnlyGladiator. Uses ArenaOnlyGladiator and any
//    arena type having AllocAligned() and Free() methods that are API
//    compatible with those of UnsafeArena.
//
//    To allocate an object of this class in the arena, use
//
//        UnsafeArena arena(1024);
//        T* obj = base::NewInArena<T>(&arena, constructor_args...);
//
//    The file also provides base::DeleteInArena<T>, which must be called before
//    releasing the arena.
//
//      base::DeleteInArena(&arena, obj);
//
//    Note that any memory allocated from inside T will still come from the heap
//    unless it is also allocated using base::NewInArena() or the class uses an
//    ArenaAllocator.
//
...
//
// WITH base::NewInArena: uses any arena type defining AllocAligned() and Free()
...
//    arena_allocator.h provides base::NewInArena and that can be used as
//    follows:
//
//      #include "gloop/base/arena_allocator.h"
//
//      UnsafeArena arena(1024);
//      Foo* foo = base::NewInArena<Foo>(&arena);
//      Foo* foo_array = base::NewInArena<Foo[]>(&arena, 10);
//
//    The file also provides base::DeleteInArena, which is necessary to call if
...
//
//      base::DeleteInArena(&arena, foo);
//      base::DeleteInArena(&arena, foo_array, 10);
//
// WITH NEW: --Uses UnsafeArena-- (DEPRECATED: use base::NewInArena as above)
//    There are cases where you can't inherit the class from Gladiator,
//    or inheriting would be too expensive.  Examples of this include
...
//     v5 = base::NewInArena<std::vector<int>>(arena_);
...
//     pva = base::NewInArena<std::vector<A>>(arena_);
//     pva2 = base::NewInArena<
...
//      // base::DeleteInArena(arena_, v4) -- not necessary arena_.Reset() will
//      // do as good
//      base::DeleteInArena(arena_, v5);  // necessary to free v5's ints memory
//      base::DeleteInArena(arena_, pva); // necessary to make sure you reclaim
//                                        // space used by A's
//      base::DeleteInArena(arena_, pva2);  // safe to call this; needed if you
...
#ifndef THIRD_PARTY_GLOOP_BASE_ARENA_H_
#define THIRD_PARTY_GLOOP_BASE_ARENA_H_
...
class BaseArena {
 ...
 public:
  ...
  // Ideas:
  //    1. kDefaultAlignment = alignof(std::max_align_t) (consistent with
  //       malloc)
  //    2. Use the alignment for the allocated type. It looks like this can be
  //       done by adjusting the definition of kAlignment in arena_allocator.h
  //       to be alignof(T).
...
class UnsafeArena : public BaseArena {
 public:
  ...
  char* Strndup(const char* s, size_t n) {
    ...
    // We can't use the standard string library since this is the base library,
...
// We declare the template classes (like we do in stl_decl)
#ifndef _ARENA_ALLOCATOR_
template <class T, class C>
class ArenaAllocator;
#endif

// astrings are strings that allocate from an UnsafeArena via an astring_alloc.
// sastrings are strings that allocate from a SafeArena via an sastring_alloc
typedef ArenaAllocator<char, UnsafeArena> astring_alloc;
typedef ArenaAllocator<char, SafeArena> sastring_alloc;
typedef std::basic_string<char, std::char_traits<char>, astring_alloc> astring;
typedef std::basic_string<char, std::char_traits<char>, sastring_alloc>
    sastring;

#endif  // THIRD_PARTY_GLOOP_BASE_ARENA_H_
