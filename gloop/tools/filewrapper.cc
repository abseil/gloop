...
std::pair<std::string, std::string> GetNamespaces() {
  ...
  if (!FLAGS_namespace.empty()) {
    const std::vector<std::string> namespaces =
        absl::StrSplit(FLAGS_namespace, "::");
    for (const auto& ns : namespaces) {
      absl::StrAppend(&intro, "namespace ", ns, " {\n");
    }
    for (auto it = namespaces.rbegin(); it != namespaces.rend(); ++it) {
      absl::StrAppend(&outro, "}  // namespace ", *it, "\n");
    }
...
std::string Comment(const std::string& base) {
  ...
  absl::StrAppend(&comment, "//    ", base, "\n");
  if (FLAGS_flatten) absl::StrAppend(&comment, "//    --flatten\n");
  for (const auto& strip : FLAGS_strip) {
    absl::StrAppend(&comment, "//    --strip ", strip, "\n");
  }
  if (FLAGS_data_in_cc) absl::StrAppend(&comment, "//    --data_in_cc\n");
  return comment;
}
...
std::string GetHeaderGuard(const std::string& base) {
  std::string guard;
  if (!FLAGS_namespace.empty()) {
    for (const auto& ns : absl::StrSplit(FLAGS_namespace, "::")) {
