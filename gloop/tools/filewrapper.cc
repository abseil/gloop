...
std::streamoff Size(std::string_view path) {
  struct stat buf;
  if (stat(std::string(path).c_str(), &buf) == -1) return 0;
  return buf.st_size;
}
...
std::string GetRunfilePath(std::string_view path) {
...
std::string TrimFront(std::string_view s, char c) {
  std::string_view::size_type pos = 0;
  while (pos < s.size() && s[pos] == c) ++pos;
  return std::string(s.substr(pos));
}

std::string TrimBack(std::string_view s, char c) {
  std::string_view::size_type pos = s.size();
...
std::string ToCIdentifier(std::string_view s) {
...
std::vector<std::string> Split(std::string_view str, std::string_view sep) {
  ...
  std::string_view::size_type pos = 0;
  for (;;) {
    std::string_view::size_type end = str.find(sep, pos);
    std::string_view::size_type len =
        (end == std::string_view::npos ? end : end - pos);
...
std::string Comment(std::string_view base) {
  std::string comment = "//  Automatically generated by tools/filewrapper\n";
  comment += "//    ";
  comment.append(base);
  comment += "\n";
...
std::string GetHeaderGuard(std::string_view base) {
  std::string guard;
  if (!FLAGS_namespace.empty()) {
    for (const auto& ns : Split(FLAGS_namespace, "::")) {
      guard += ns;
...
void WriteHeader(std::string_view filename, std::string_view comment,
                 const std::pair<std::string, std::string>& namespaces,
                 std::string_view base) {
  std::ofstream hdr(std::string(filename),
                    std::ios_base::out | std::ios_base::trunc);
  if (!hdr.is_open()) {
    FatalError("Unable to open header file '%s' for writing",
