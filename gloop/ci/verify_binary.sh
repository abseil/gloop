...
case "$MODE" in
  arch)
    ...
    readelf -h "$BINARY" | grep "$PATTERN" >/dev/null || {
  ...
  symbol)
    ...
    nm "$BINARY" | grep "$PATTERN" >/dev/null || {
  ...
  needed)
    ...
    readelf -d "$BINARY" | grep "$PATTERN" >/dev/null || {
  ...
  not_needed)
    ...
    if readelf -d "$BINARY" | grep "$PATTERN" >/dev/null; then
