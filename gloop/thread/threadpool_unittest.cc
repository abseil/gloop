...
static void ValidateAddAfter(int64_t run_time, const char* prefix,
                             absl::Notification* done) {
  // Callback must've been executed in expected thread.
  const LiveThread* th = Thread_GetMyLiveThread();
  CHECK(th) << "Callback executed in an unknown thread";
  CHECK_EQ(strncmp(LiveThread_Name(th), prefix, strlen(prefix)), 0)
      << "Callback executed in an unknown thread, expected thread prefix "
      << prefix << ", found " << LiveThread_Name(th);

  int64_t now = absl::ToUnixMillis(absl::Now());
  CHECK_GE(now, run_time) << "Callback invoked too early";
  done->Notify();
}

TEST(ThreadPoolTest, AddAfter) {
  ThreadPool pool(1, {.name_prefix = "foo"});

  int ms = 123;
  absl::Notification done;
  auto c = absl::bind_front(
      &ValidateAddAfter,
      absl::ToUnixMillis(absl::Now() + absl::Milliseconds(ms)), "foo/", &done);
  pool.ScheduleAt(absl::Now() + absl::Milliseconds(ms), c);

  // Expect callback to be executed no more than a threshold past the given
  // deadline.
#ifdef __APPLE__
  // We observe scheduling delays of up to 150ms on macOS.
  const absl::Duration threshold = absl::Milliseconds(200);
#else
  const absl::Duration threshold = absl::Milliseconds(100);
#endif
  CHECK(done.WaitForNotificationWithTimeout(threshold + absl::Milliseconds(ms)))
      << "Callback didn't execute within threshold";
}
