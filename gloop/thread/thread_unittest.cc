...
TEST(ThreadTest, CheckForEach) {
  ...
  EXPECT_EQ(in_each_collector.Count(), 0);
...
INSTANTIATE_TEST_SUITE_P(ThreadTest, ThreadNameTest,
                         testing::Values(std::nullopt, "B0_b"));
...
INSTANTIATE_TEST_SUITE_P(
    ThreadTest, SanitizeThreadNamePrefixTest,
...
TEST(ThreadTest, IOPriorityAppliedToThread) {
  thread::Options options;
  // Best Effort class (2) with level 5 is standard and safe.
...
TEST(ThreadDeathTest, UnjoinedJoinableThreadWarning) {
  EXPECT_DEATH_IF_SUPPORTED(
      {
        {
          absl::Notification thread_done;
          ClosureThread t([&thread_done] { thread_done.Notify(); });
          t.SetJoinable(true);
          t.Start();
          thread_done.WaitForNotification();
          absl::SleepFor(absl::Milliseconds(50));
        }
        abort();
      },
      "Joinable thread was not joined");
}
