...
namespace base {
inline bool StackTraceWorksForTest() { return true; }
}  // namespace base
...
// This is a hack.  perftools/tracing/public/... is not itself portable so we
// must use it conditionally.
#if !PORTABLE_BASE
...
std::string TruncateThreadNameScope(absl::string_view name) {
  if (name.substr(0, 12) == "third_party_") {
    return std::string(name.substr(12));
  }
...
static bool ShouldRunSignalTest(const char* testname) {
  if (!base::StackTraceWorksForTest()) {
...
TEST(ThreadTest, CheckPeriodicThreads) {
  ...
  fflush(stdout);
  CHECK(signaled);
  CHECK_LT(timer.Get(), 1.0);  // shouldn't have had to wait at all
...
TEST(ThreadTest, CheckThreadNameUnnamed) {
  ...
  CHECK(collector.ContainsNamePrefix(
      TruncateThreadNameScope("third_party_gloop_thread_thread_unittest")));
  CHECK(collector.ContainsNamedThread(
