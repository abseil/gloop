...
ABSL_RETIRED_FLAG(bool, fibers_use_cooperative_scheduling, false, "Retired.");
...
ABSL_FLAG(
    ...
    "not already joinable. This flag is intended for rollback only- if you "
    "feel you need to set it, you must first file a bug "
    "to help debug your issue. It will be removed shortly.");
...
void Fiber::Body() {
  ...
  *ptr = old;
  ...
  if (CleanupFn* const clean_up = MarkFinished()) {
    // This is a self-joining fiber.
    InternalJoin();
    clean_up(this);
  }
}
...
Fiber::CleanupFn* Fiber::MarkFinished() {
  absl::MutexLock l(mu_);
