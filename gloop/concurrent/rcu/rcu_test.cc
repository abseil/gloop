...
TEST_F(RcuTest, MultipleQueueCallsOnBlockedDomain) {
  ...
  // result in duplicate entries in Gloop's active cleanup list.
  // Any duplicate queue requests should be merged, keeping the active queue
  // count (num_queues_) at exactly 1.
  // We sleep briefly to allow the background thread to finish processing the
  // final pop before asserting the count.
